...
void main() {
  ...
  const String path = "${AppConstants.baseUrl}/api/v1/fcm";
  late DioAdapter mockDioAdapter;
  late MockDio mockDioClient;
  late FirebaseRepo dataSource;
  ...
  setUpAll(() async {
    ...
    dataSource = FirebaseRepo(dioClient: mockDioClient);
  });

  group('Firebase Api Test (repository)', () {
    group("Init FCM", () {
      test('Should throw internal server error when fetch process is failed',
        ...
        //act
        final call = dataSource.initFcm(token: any, userId: any, lat: any, lng: any);
