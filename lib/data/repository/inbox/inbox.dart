...
class InboxRepo {
  ...
  Future<InboxModel?> getInbox({required String userId, required String type, int? page = 1}) async {
    try {
      Response res = await dioClient!.post("${AppConstants.baseUrl}/api/v1/inbox?page=$page&limit=6", data: {
        "user_id": userId,
        "type": type,
      });
  ...
  Future<InboxPaymentModel?> getInboxPayment({required String userId, int? page = 1}) async {
    try {
      Response res = await dioClient!.post("${AppConstants.baseUrlPpob}/inbox?page=$page&limit=6", data: {
        "user_id": userId,
        "origin": "hp3ki",
      });
  ...
  Future<InboxCountPaymentModel?> getInboxCountPayment({required String userId}) async {
    try {
      Response res = await dioClient!.post("${AppConstants.baseUrlPpob}/inbox/badges", data: {
        "user_id": userId,
        "origin": "hp3ki",
      });
