...
class PPOBProvider with ChangeNotifier {
  ...
  List<DenomPulsaData> get listVoucherPulsaByPrefixData => _listVoucherPulsaByPrefixData;
  ...
  SelectedPaymentMethodData? get selectedPaymentMethodData => _selectedPaymentMethodData;
  ...
  SelectedContactUserStatus _selectedContactUserStatus = SelectedContactUserStatus.loading;
  SelectedContactUserStatus get selectedContactUserStatus => _selectedContactUserStatus;
  ...
  void setStateSelectedContactUserStatus(SelectedContactUserStatus selectedContactUserStatus) {
  ...
  void setStateListWalletDenomStatus(ListWalletDenomStatus listWalletDenomStatus) {
  ...
  Future<void> purchasePulsa(BuildContext context, {required String productCode, required String phone}) async {
    setStatePayStatus(PayStatus.loading);
    try {
      await pr.payPulsa(productCode: productCode, phone: phone, userId: SharedPrefs.getUserId());
      setStatePayStatus(PayStatus.loaded);
  ...
  Future<void> inquiryTopUp(BuildContext context, String productId, String paymentChannel) async {
    setStatePayStatus(PayStatus.loading);
    try {
  ...
  Future<void> postInquiryPLNPrabayarStatus(BuildContext context, {
    ...
    try {
      InquiryPLNPrabayarModel? inquiryPLNPrabayarModel = await pr.postInquiryPLNPrabayar(idPel: idPel);
  ...
  Future<void> postInquiryPLNPascaBayar(BuildContext context, String accountNumber) async {
    ...
    try {  
      ...
      if(e.type == DioErrorType.response) {
        if(e.response!.statusCode == 500 || e.response!.statusCode == 411) {
          debugPrint("Internal Server Error (Post Inquiry PLN Pascabayar Status)");
        } 
        if(e.response!.statusCode == 404) {
          debugPrint("Internal Server Error (URL not found)");
        }
  ...
  Future<void> getListPricePLNPrabayar(context) async {
    ...
    try {
      ListPricePraBayarModel listPricePraBayarModel = await pr.getListPricePLNPrabayar();
  ...
  Future<void> getVoucherPulsaByPrefix(BuildContext context, {required int prefix, required String type}) async {
    setStateListVoucherPulsaByPrefixStatus(ListVoucherPulsaByPrefixStatus.loading);
    try {
      DenomPulsaModel? listVoucherPulsaByPrefixModel = await pr.getDenomPulsa(prefix: prefix.toString(), type: type);
  ...
  Future<void> getBalance(BuildContext context) async {
    ...
    try {
      WalletBalanceModel? balanceModel = await pr.getWalletBalance(userId: SharedPrefs.getUserId());
  ...
  Future<void> updateUsers({
    ...
    try {  
      ...
      await DBHelper.update('contacts', data: {
        "name": name,
        "phone": phone
      });
      Future.delayed(Duration.zero, () {
  ...
  Future<void> insertUsers({required String name, required String phone}) async {
    try {
      await DBHelper.resetUsersStatus('contacts');
      await DBHelper.insert('contacts', data: {
        "id": "a",
        "name": name,
        "status": "true",
        "phone": phone
  ...
  void setPaymentMethod({required String id, required String name, required String channel}) {
    isSelected = true;
